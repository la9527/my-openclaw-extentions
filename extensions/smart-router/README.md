...
5. 응답 본문 대신 OpenClaw 채팅 footer model 메타에 현재 route alias(`local`, `nano`, `mini`, `full`)를 표시할 수 있습니다.
6. 생성 단계에서 즉시 `response_error`가 나거나 첫 토큰이 일정 시간 안에 오지 않으면 상위 tier로 1회 재시도할 수 있습니다.
7. `llm` 분류가 실패하면 원인별 fallback을 적용합니다: timeout이 3초 이상이면 `full`, 연결/HTTP/JSON 실패는 `nano`.
...
실제 실행 예시는 [../../configs/openclaw-hybrid.json5](../../configs/openclaw-hybrid.json5) 를 보면 됩니다.
...
| `evaluationLlmTarget` | `nano` | `llm` 평가용 target (`local`, `nano`, `mini`, `full`) |
| `evaluationLlmModel` | 없음 | `llm` 평가용 모델 override |
| `evaluationTimeoutMs` | `15000` | `llm` 평가 타임아웃 |
| `evaluationTimeoutRetryCount` | `1` | `llm` 평가 타임아웃 재시도 횟수 |
| `evaluationTimeoutFallbackTarget` | `nano` | timeout이 3초 미만인 경우에만 적용되는 타임아웃 fallback tier |
| `streamFirstTokenTimeoutMs` | `15000` | 생성 스트림의 첫 토큰 대기 타임아웃. 초과 시 상위 tier로 1회 재시도 |
| `streamRetryOnFailure` | `true` | 생성 단계 `response_error`/first-token timeout 시 상위 tier 1회 재시도 여부 |
| `showModelLabel` | `true` | OpenClaw 채팅 footer model 메타를 route alias(`local`, `nano`, `mini`, `full`)로 치환할지 여부 |
...
`evaluationMode=llm` 에서 현재 권장 정책은 아래와 같다.

1. 기본 분류 경로는 `evaluationLlmTarget=local`
2. 분류 timeout이 `3초 이상`이면 `full`로 강제 fallback
3. 분류 JSON/연결/HTTP 실패는 `nano`로 강제 fallback
