...
| `moderate` | `nano` | `gpt-5.4-nano-2026-03-17` |
...
4. `nano`는 기본 auto 라우팅의 중간 tier 이면서 `llm` 분류 모델 용도로도 사용할 수 있습니다.
...
1. 가장 안정적으로 쓰려면 `rule`
2. `nano`를 auto 중간 tier와 분류 모델로 함께 쓰려면 `llm` + `evaluationLlmTarget: nano`
...
3. 다만 `경보 조건 3개`처럼 alert-only 운영 요청은 아직 `advanced/full` 로 갈 수 있다.
...
| `nanoModel` | `gpt-5.4-nano-2026-03-17` | auto 라우팅 `moderate` tier, `llm` 분류 모델, 직접 nano 선택 모델 |
| `miniModel` | `gpt-5.4-mini-2026-03-17` | auto 라우팅 `complex` tier 모델 |
...
| `threshold` | `moderate` | 기본값일 때 `0~3 local`, `4~6 nano`, `7~11 mini`, `12+ full` |
...
2. `nano`는 moderate 기본값이라기보다 저비용 remote tier 또는 평가용 LLM 역할이 더 자연스럽다.
3. `llm` 분류는 계속 튜닝이 필요하며, 특히 짧은 운영 지표 질의와 간단한 모델 설명 질의에 대한 과승격/과원격화를 더 줄여야 한다.
...
[smart-router] ☁️ moderate (score: 4, eval: llm) → nano:gpt-5.4-nano-2026-03-17 | 일반 설명 요청
