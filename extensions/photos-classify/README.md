...
/classify local /Users/me/Pictures/family --profile person
...
- `/classify-status` 에 job_id 없이 입력하면 최근 job 진행률, stage, profile 요약을 채팅에서 바로 볼 수 있다.
...
- 사람 중심 선별은 `--profile person`, 풍경 중심 선별은 `--profile landscape` 또는 MCP의 `selection_profile` 인자를 사용한다.
